...
function DemoChess() {
  ...
  const handleUpdate = async (evt) => {
    ...
    const data = {
      ...
      await axios.put(`https://crm.supersiteuz.com/orders/${tempEvent.id}`, data)
  ...
  const HandleChange =  async(e) => {
    ...
    await axios.put(`https://crm.supersiteuz.com/orders/${tempEvent.id}` , body)
     .then(res => res)
     window.location.reload();
  }
  ...
  const HandleOrderDelete = async() => {
   await axios.delete(`https://crm.supersiteuz.com/deleteorders/${tempEvent.id}`)
   .then(res => res)
    window.location.reload()
  } 
  const HandleOrder = () =>{
...
.slice(0 , 10))}</span>
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs} >
      <DemoContainer  components={['DateTimePicker', 'DateTimePicker']}>
        ...
        />
          ...
          <Input
            style={{ width:"100%",backgroundColor:"#F5F5F5", color:"black" , height:"50px" , borderRadius :"5px" }}
            value={NumberRights}
            placeholder={0}
          />
              </div>
  ...
  <div className={styles.inputdiv}>
        ...
        <span value={elem.id} style={{marginLeft:"3px",marginRight:"3px"}}>{elem.type}</span> |
        <span value={elem.id} style={{marginLeft:"3px"}}>{elem.count} <i value={elem.id} class='bx bxs-user' style={{position:"relative" , top:"1px"}}></i></span>
        </option>
      )}
          </select>
  </div>  
  ...
  <Button className="mbsc-button-block" onClick={HandleEditOpen} color="warning">
        ...
        <div style={{width:"93%" , margin:"0 auto" , display :"flex" }}>
            <Button style={{width:"50%"}} className="mbsc-button-block" onClick={HandleOrderEdit} color="secondary-color">
            {t("сhess.16")}
          </Button>
          <Button style={{width:"50%"}} className="mbsc-button-block" onClick={HandleOrderDelete} color="danger">
            {t("сhess.18")}
          </Button>
            </div>
        <div className="mbsc-button-group">
