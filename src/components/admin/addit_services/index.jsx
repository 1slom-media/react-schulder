...
const AdditServiesComponent = () => {
  ...
  const HandleSubmitMiniBar = async (e) => {
    ...
    await  axios.put(`https://crm.supersiteuz.com/products/${products}` , bodys)
    .then(res => res)
    let bodyPrices = {
      paid:
    ...
    if (statusPayment == 'Оплачено') {
      await  axios.put(`https://crm.supersiteuz.com/orders/${orders}` , bodyPrices)
      .then(res => res)
    }else{
      await  axios.put(`https://crm.supersiteuz.com/orders/${orders}` , bodyPrices2)
      .then(res => res)
    }
