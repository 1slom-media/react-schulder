...
import { OldOrdersPut } from '../../../../../redux/old_orders';
import { OrdersGet, OrdersPut } from '../../../../../redux/orders';
...
const Refund = ({open , HandleClose}) => {
  ...
  const data = JSON.parse(window.localStorage.getItem("AuthDataUser"));
    ...
    const HandleSubmit = async () =>{
      ...
      await  axios.put(`https://crm.supersiteuz.com/oldorders/${dataFind.map(elem => elem.old.map(e => e.id)[0])[0]}` , body3)
      .then(res => res)
      if (dataFind.map(elem => elem.debt) > 0) {
        await  axios.put(`https://crm.supersiteuz.com/orders/${id}` , body)
        .then(res => res)
      }else {
        await  axios.put(`https://crm.supersiteuz.com/orders/${id}` , body2)
        .then(res => res)
      }
      await dispatch(
        ChangePost({
